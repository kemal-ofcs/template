...
fn require_session(state: &MobileState) -> Result<OperatorUser, CommandError> {
...
fn clear_expired_session(state: &MobileState, error: &CommandError) {
...
pub fn desktop_get_runtime_status(
    state: State<'_, MobileState>,
) -> Result<MobileRuntimeStatus, CommandError> {
    let session = state.session.lock().map_err(|_| CommandError::internal())?;
...
fn ensure_bootstrap_window_open(state: &MobileState) -> Result<(), CommandError> {
    if state
        .session
        .lock()
        .map_err(|_| CommandError::internal())?
...
) -> Result<MobileLoginResult, CommandError> {
...
#[tauri::command]
pub async fn desktop_logout(state: State<'_, MobileState>) -> Result<(), CommandError> {
    let previous = state
        .session
        .lock()
        .map_err(|_| CommandError::internal())?
...
#[tauri::command]
pub async fn desktop_get_roles(state: State<'_, MobileState>) -> Result<Value, CommandError> {
...
#[tauri::command]
pub fn desktop_get_sync_conflicts(state: State<'_, MobileState>) -> Result<Value, CommandError> {
    require_permission(&state, "sync.view")?;
...
/// ke tombol Bagikan.
#[tauri::command]
pub fn desktop_get_data_folder(state: State<'_, MobileState>) -> Result<Value, CommandError> {
