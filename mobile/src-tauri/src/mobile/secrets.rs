...
fn credential_paths(
    ...
    Ok((
        directory.join(format!("{identity_key}.vault")),
        directory.join(format!("{identity_key}.salt")),
    ))
}

fn derive_key(password: &str, salt_path: &Path) -> Result<[u8; 32], CommandError> {
...
fn write_snapshot(
    ...
    let temporary = snapshot_path.with_extension("vault.new");
    let backup = snapshot_path.with_extension("vault.bak");
    ...
    fs::write(&temporary, file_payload).map_err(|_| CommandError::internal())?;
...
pub fn load_offline(
    ...
    let stored_identity_key =
        ...
        "Perangkat mobile ini wajib login online minimal satu kali sebelum dapat digunakan offline.",
      )
            },
        )?;
    ...
    let payload = fs::read(&snapshot_path).map_err(|_| CommandError::internal())?;
    if payload.len() < 12 + 16 {
        return Err(CommandError::new(
            "OFFLINE_SNAPSHOT_INVALID",
            "Snapshot keamanan offline rusak atau tidak lengkap.",
        ));
    }
    ...
    if credential.server_origin != server_origin || !identifier_matches || (!valid_v1 && !valid_v2)
    {
        return Err(CommandError::new(
            "OFFLINE_SNAPSHOT_INVALID",
            "Snapshot keamanan offline tidak cocok dengan server deployment ini.",
        ));
    }
