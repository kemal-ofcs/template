...
fn load_table_cursors(state: &MobileState) -> Result<HashMap<String, i64>, CommandError> {
    let connection = storage::database(&state.data_dir)?;
...
pub async fn pull_snapshot(state: &MobileState) -> Result<MobileSyncStatus, CommandError> {
...
fn mark_batch_failed(state: &MobileState, event_ids: &[String], message: &str) {
...
fn ensure_unsynced_local_data_enqueued(_state: &MobileState) -> Result<(), CommandError> {
    Ok(())
}
...
pub async fn push_outbox(state: &MobileState) -> Result<(), CommandError> {
...
pub async fn synchronize(state: &MobileState) -> Result<MobileSyncStatus, CommandError> {
    if SYNC_IN_FLIGHT
...
async fn enforce_rbac_revision(state: &MobileState) {
...
pub fn status(state: &MobileState) -> Result<MobileSyncStatus, CommandError> {
    let client_id = ensure_client_id(state)?;
    let connection = storage::database(&state.data_dir)?;
...
pub fn conflicts(state: &MobileState) -> Result<Value, CommandError> {
    let connection = storage::database(&state.data_dir)?;
    let mut statement = connection
        .prepare(
            r#"
...
pub fn retry_failed(state: &MobileState, event_id: Option<&str>) -> Result<(), CommandError> {
    let connection = storage::database(&state.data_dir)?;
...
mod tests {
    ...
    use super::{storage, MobileState, CANONICAL_SYNC_ROUTES, SNAPSHOT_TABLES};
    ...
    fn status_menandai_mode_lokal_hanya_untuk_provider_local_file() {
        ...
        for (provider, harapan) in [
            (crate::mobile::turso::DatabaseProvider::Turso, false),
            (crate::mobile::turso::DatabaseProvider::SelfHosted, false),
            (crate::mobile::turso::DatabaseProvider::LocalFile, true),
        ] {
            *state.turso_config.write().expect("kunci config") =
                Some(crate::mobile::turso::TursoConfig::new(
